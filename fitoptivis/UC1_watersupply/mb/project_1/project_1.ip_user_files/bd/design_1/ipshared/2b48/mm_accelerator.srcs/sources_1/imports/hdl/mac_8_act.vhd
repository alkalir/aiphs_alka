...
architecture FSM of mac_8 is
    type t_enum2 is (MUL,ACC,SEND);
    ...
    signal n_idx : unsigned(2 downto 0);
...
begin
  ...
  begin
    ...
    if in_data_empty='0' and state=MUL then
      ...
      n_idx <= idx;
      en_idx <= false;
    -- state=ACC, idx = 7, out_data.rdy / state:=MUL, p_acc:=0, wr(out_data,acc+prod), idx:=0
    elsif state=ACC and ((idx) = (to_unsigned(7,3))) and out_data_full='0' then
      n_state <= MUL;
      en_state <= true;
